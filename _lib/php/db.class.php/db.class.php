  ...
  class DB
  {
    ...
    /** Connect to a MySQL database to be able to use the methods below.
      */
		...
		var $remote_cred = array(
		'base' => 'robriot',
		'server' => 'robriot.db.5419123.hostedresource.com',
		'user' => 'robriot',
		'pass' => 'Design126!'
		);		
	...
	function DB(){
      ...
      mysql_connect($logincred['server'], $logincred['user'], $logincred['pass']) or die('Server connexion not possible.');
      mysql_select_db($logincred['base']) or die('Database connexion not possible.');
    }
    ...
    function execute($query, $debug = -1)
    {
      ...
      $this->debug($debug, $query.$this->lastInsertedId());
    }
    ...
    /** Return the string as a safe sting to insert.
      * @Return the string as a safe sting to insert.
      */
    function safe($str)
    {
